            ...
            for item in journey_type:
                ...
                for leg_index, leg in enumerate(item['legs']):
                    leg_info = {
                        ...
                        'modalityOperator': leg.get('operatorName'),
                    ...
                    # if leg_info['modality'] != 'Walking': //why excluding walking info?
                    for call in leg['calls']:
                        call_info = {
                            'departureTime': call.get('departureTime'),
                            'locationId': call.get('location', {}).get('id'),
                            'displayName': call.get('location', {}).get('displayName'),
                            'type': call.get('location', {}).get('type'),
                            'latitude': call.get('location', {}).get('latitude'),
                            'longitude': call.get('location', {}).get('longitude'),
                            'placeId': call.get('location', {}).get('placeId'),
                            'placeName': call.get('location', {}).get('placeName'),
                            'placeLatitude': call.get('location', {}).get('placeLatitude'),
                            'placeLongitude': call.get('location', {}).get('placeLongitude'),
...
output_file = 'journey_info_1.json'
